...
const ProductCard = ({ id, image, name, price }) => {
	...
	const handleAddToCart = () => {
		// Dimensiunea implicită (50 ml)
		const size = '50 ml'
		// Adaugă produsul în coș cu dimensiunea și cantitatea implicită
		addToCart(id, size)
	}
	...
	return (
		<div
			...
			w-full sm:w-64 md:w-72 lg:w-80 h-auto'
		>
			{/* Imaginea produsului */}
			<Link to={`/product/${id}`} onClick={() => scrollTo(0, 0)}>
				<div
					className='flex items-center justify-center overflow-hidden rounded-xl w-full sm:w-48 md:w-56 lg:w-64 h-40 sm:h-44 md:h-52'
				>
					<img
						src={image[0]}
						alt={name}
			...
			<div className='w-full text-center mt-4'>
				<p className='text-lg sm:text-xl font-semibold text-gray-800 mb-2 break-words'>
					{name}
				</p>
			</div>
